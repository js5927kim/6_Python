...
result2 = demo.groupby('code')['close'].transform(lambda s: s.interploate())
print(result2)  

"""
    groupby 없이 보간처리를 하면 마지막 종가(G0001)와 뒷 종목의 다음 종가 사이를 이어버림
    groupby 를 사용하여 보간처리를 하면 해당 종목(그룹화기준열) 안에서만 이어짐
"""
print("=" * 60)
...
for col in OHLC:
    ...
    # .gtill() : 결측을 바로 뒤의 값으로 채움
    for col in OHLC:
        df[col] = df.groupby('code')[col].transgorm(lambda s: s.ffill().bfill())
