...
#   날짜 인덱스로 슬라이싱 할 때는, 이전에 정렬이 되어있어야 함
...
for idx in monthly_last.index[:4]:
    print(f"{idx.strftime('%Y-%m'):<12} {monthly_last[idx]:>16.0f}")

    print(f"월별 거래량 : {monthly_vol.iloc[0]:,.0f}")

    # * resample("ME").ohlc()
    #   ohlc : Open High Low Close
    # => 기간별 시가, 고가, 저가, 종고 네개열을 한번에 만들수있음
    print(one['close'].resample('ME').ohlc().head(3).round(0))

    # rolling : 연속된 N개 행을 윈도우 단위로 홅으면서 계산
    ...
    for i in [edge-1, edge, edge+1]:
        ...
        print(f"{i:<8} {df.loc[1, 'code']:<9} {df.loc[1, 'close']:>10} {w:>20} {r:>20}")


"""
    groupby 없이 계산하면 (wrong) 종목별 코드가 달라도 이평선 재계산없이 이어지는 형태
...
# 누적 수익률
# => 첫 날의 NaN으로 0으로 채우고, (1+수익률)을 차례로 곱하기
