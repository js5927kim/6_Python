...
# csr : client side rendering. 빈 HTML을 서버로부터 응답받고, 자바스크립트를 통해 데이터를 화면에 표시.
...
with sync_playwright() as p:
    # browser : 크롬을 실행
    browser = p.chromium.launch(headless=True) # hedless=True => 창이 보이지 않음
    ...
    # page.route(패턴, 처리함수) : 특정 패턴의 요청을 가초래서 직접 처리하는 함수
    # route.abort()     : 요청을 취소
